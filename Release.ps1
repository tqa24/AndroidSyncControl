...
# (git tag M.N.0; git push origin M.N.0). CI builds M.N.<commits-since-tag>, attaches the app
# zip (AndroidSyncControl-<version>-<TargetFramework>.zip) to the GitHub Release 'M.N.0'.
...
if ([string]::IsNullOrWhiteSpace(($staged | Out-String))) {
    ...
    Write-Host "To re-run CI without a new commit: gh workflow run release.yml --ref <branch>"
    return
}
